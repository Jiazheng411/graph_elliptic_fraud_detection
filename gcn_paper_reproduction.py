"""
GCN Paper Reproduction - Exact Setup
====================================
Reproducing the exact setup from the Elliptic paper:
1. Train/Test split only (train = original train + val)
2. 2-layer GCN with 100-dim embeddings  
3. Weighted CrossEntropyLoss (0.3/0.7)
...
def train_gcn(train_data, test_data, include_unknowns, num_epochs=2000, shared_timestamp=None):
    """Train GCN with paper's exact hyperparameters and save checkpoints"""
    ...
    experiment_type = "gcn_with_unknowns" if include_unknowns else "gcn_labeled_only"
    checkpoint_dir = f"checkpoints/gcn/{timestamp}/{experiment_type}"
    os.makedirs(checkpoint_dir, exist_ok=True)
    
    print(f"Saving checkpoints to: {checkpoint_dir}")
    
    # Model setup - 2-layer GCN with 100-dim hidden
    input_dim = train_data.x.shape[1]
    model = GCN(input_dim, hidden_dim=100, num_classes=2).to(device)
    
    # Adam optimizer with lr=0.001 as in paper
    ...
    print(f"\nTraining GCN (Paper Reproduction) - {num_epochs} epochs...")
    ...
    for epoch in range(num_epochs):
        ...
        # Evaluation every 50 epochs for progress monitoring
        if epoch % 50 == 0:
            model.eval()
            with torch.no_grad():
    ...
    # Save loss history as JSON instead of .pt
    ...
    # Save summary results as JSON instead of .pt
    summary = {
        'experiment_type': 'with_unknowns' if include_unknowns else 'labeled_only',
        'timestamp': timestamp,
        'num_epochs': num_epochs,
        'best_epoch': best_epoch,
        'best_metrics': best_test_metrics,
...
def main():
    print("GCN Paper Reproduction")
    print("======================")
    print("Reproducing exact setup from Elliptic paper:")
    print("- Train/Test split (combining train+val as train)")
    print("- 2-layer GCN, 100-dim embeddings")
    print("- Weighted CrossEntropyLoss (0.7 illicit, 0.3 licit)")
    print("- 2000 epochs, Adam lr=0.001")
    ...
    print(f"All results will be saved under: checkpoints/gcn/{shared_timestamp}/")
    ...
    best_metrics_labeled, final_metrics_labeled, history_labeled, checkpoint_dir_labeled = train_gcn(train_graph, test_graph, include_unknowns=False, num_epochs=1500, shared_timestamp=shared_timestamp)
    ...
    best_metrics_unknowns, final_metrics_unknowns, history_unknowns, checkpoint_dir_unknowns = train_gcn(train_graph, test_graph, include_unknowns=True, num_epochs=1500, shared_timestamp=shared_timestamp)
    ...
    results_dir = f"checkpoints/gcn/{shared_timestamp}/comparisons"
    os.makedirs(results_dir, exist_ok=True)
    results_file = os.path.join(results_dir, "gcn_paper_reproduction_results.json")
    
    # Add metadata to results
    results['metadata'] = {
        'experiment_name': 'GCN Paper Reproduction',
        'timestamp': shared_timestamp,
    ...
    print(f"All results saved under: checkpoints/gcn/{shared_timestamp}/")
    print(f"")
    print(f"Folder structure:")
    print(f"  📁 checkpoints/gcn/{shared_timestamp}/")
    print(f"    ├── 📁 gcn_labeled_only/     (individual experiment results)")
    print(f"    ├── 📁 gcn_with_unknowns/    (individual experiment results)")
    print(f"    └── 📁 comparisons/          (comparative analysis)")
    print(f"         └── gcn_paper_reproduction_results.json")
    print(f"")
